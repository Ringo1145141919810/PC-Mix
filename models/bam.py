...
class BAM(nn.Module):
    def __init__(self, args, config) -> None:
        ...
        self.config = config
        self.args = args
        print("config:v"+config.ssl_ckpt)

        # self.ssl_layer = getattr(hub, config.ssl_name)(ckpt=config.ssl_ckpt, fairseq=True)
        self.ssl_layer = getattr(hub, config.ssl_name)(ckpt="/dkucc/home/zz324/partComSpoof/BAM-master/models/WavLM-Large.pt", fairseq=True)
